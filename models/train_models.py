...
This script now also records the exact library versions used during training
to ensure reproducibility, saving them to `training_report.json` and
...
def get_library_versions():
    """Returns a dictionary of key library versions for reproducibility."""
...
# ---------------------------------------------------------------------------
# Final reporting: capture environment and write artifacts
# ---------------------------------------------------------------------------
library_versions = get_library_versions()
REPORT["_environment"] = library_versions

# Write a requirements file for the model training environment for perfect reproducibility
model_req_path = os.path.join(MODEL_DIR, "requirements.txt")
with open(model_req_path, "w") as f:
    lines = [
        "# This file was auto-generated by train_models.py to capture the training environment.",
        "# Use this to create a virtual environment for re-training:",
        "# python -m venv .venv && source .venv/bin/activate && pip install -r models/requirements.txt",
        "",
        f"scikit-learn=={library_versions.get('scikit-learn', 'Not found')}",
        f"pandas=={library_versions.get('pandas', 'Not found')}",
        f"numpy=={library_versions.get('numpy', 'Not found')}",
        ""
    ]
    f.write("`\n`".join(lines))
...
print(f"Training environment requirements saved to {model_req_path}")